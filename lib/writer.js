...
class Writer {
  ...
  write() {
    ...
    if (this.opt.single) {
      this.log.verbose("Writing Single file...");
      return this.createSingleFile();
    }
    this.log.verbose("Writing Multi files...");
  ...
  createOutputFiles() {
    ...
    this.fs.writeFileSync(
      this.path.join(this.opt.dest, "index.html"),
      this.getIndexContent()
    );

    // create assets folder
    const assetsPath = this.path.resolve(
      this.path.join(this.opt.dest + "assets")
    );
    ...
    this.fs.copySync(
      this.findAsset("bootstrap/dist/css/bootstrap.min.css"),
      this.path.join(assetsPath, "bootstrap.min.css")
    );
    this.fs.copySync(
      this.findAsset("bootstrap/dist/css/bootstrap.min.css.map"),
      this.path.join(assetsPath, "bootstrap.min.css.map")
    );
    this.log.verbose("Copying prism css to: " + assetsPath);
    this.fs.copySync(
      this.findAsset("prismjs/themes/prism-tomorrow.css"),
      this.path.join(assetsPath, "prism.css")
    );
  ...
  runWebpack(outputPath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const webpackConfig = require(this.path.resolve(
      ...
      compiler.run((err, stats) => {
        if (stats?.compilation?.errors && stats.compilation.errors.length > 0) {
          ...
          stats.compilation.errors.forEach((e) => {
            errWP.push(e.message.substring(0, 200));
          });
          this.log.error(`Webpack ${errWP.length} errors`, errWP.join("\n"));
          return reject(new Error(errWP.join("\n")));
        }
        if (err) {
  ...
  createSingleFile() {
    ...
    const prismCss = this.fs.readFileSync(
      this.findAsset("prismjs/themes/prism-tomorrow.css"),
      "utf8"
    );
    const mainCss = this.fs.readFileSync(
      this.path.join(this.opt.template, "src", "css", "main.css"),
      "utf8"
    );
    ...
    return this.runWebpack(tmpPath)
      .then((tmpPath) => {
        const mainBundle = this.fs.readFileSync(
          this.path.join(tmpPath, "main.bundle.js"),
          "utf8"
        );
      ...
      .catch((err) => {
        console.error("Erro no webpack", err);
      });
  }
